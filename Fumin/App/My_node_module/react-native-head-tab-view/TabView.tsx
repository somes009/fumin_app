...
export default class TabView<T> extends React.PureComponent<TabViewProps<T> & typeof defaultProps, TabViewState<T>> {
    ...
    */
    ...
    _onHandlerStateChange = ({ nativeEvent }: PinchGestureHandlerEventExtra) => {
    ...
    _renderTabBar() {
        const { renderTabBar , underTabComponent} = this.props;
    ...
    */
    ...
    scrollTo(y) {
        // this.refList.updateView({value: 0})
    ...
    makeSceneParams(item: T, index: number) {
        const { makeHeaderHeight, faultHeight, renderScrollHeader, frozeTop, isRefreshing, onStartRefresh, slideAnimated, onTabViewScroll } = this.props;
    ...
    makeTabParams() {
        ...
        const { tabbarStyle, lineStyle, activeItemStyle, inactiveItemStyle, renderTabItem, renderScrollHeader, frozeTop, tabs, tabNameConvert, averageTab, tabsContainerStyle, activeTextStyle, inactiveTextStyle, isRefreshing, refreshHeight, slideAnimated } = this.props
...
const Scene = (props: SceneProps) => {
    ...
    return <View {...props}>
        <SceneView   ref={props.getRef}shouldUpdate={props.shouldUpdate}>
            {props.children}
        </SceneView>
    </View>
}
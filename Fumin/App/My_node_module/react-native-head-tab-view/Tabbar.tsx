...
export default class Tabbar<T> extends React.PureComponent<TabbarProps<T>, TabbarState> {
    ...
    */
    needsScrollTab() {
        ...
        if (!this.tabbarFrame[activeIndex]) return;
    ...
    */
    updateView(offset: { value: number }) {
        ...
        if(!(offset.value % 1)) {
            this.toScrollTab?.(this.tabFrames[offset.value - 1 < 0 ? 0 : offset.value - 1]?.left)
    ...
    */
    renderTabItem({ item, index, onLayoutTab }: TabItemInfo<T>) {
        ...
        const { activeIndex, activeTextStyle, goToPage, inactiveTextStyle, scrollValue, tabItemStyle, tabNameConvert, tabs, renderTabItemButton, activeItemStyle, inactiveItemStyle } = this.props;
        ...
        const ItemStytle = isActive ? activeItemStyle : inactiveItemStyle
        ...
        >
            {renderTabItemButton ? renderTabItemButton({ item, index ,isActive}) : <Animated.View style={[{ opacity }, ItemStytle]}>
                <Text style={[textStytle]}>{tabNameConvert ? tabNameConvert(item) : item}</Text>
            </Animated.View>}
        </Button>
    }
